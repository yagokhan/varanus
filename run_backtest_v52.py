#!/usr/bin/env python3
"""
run_backtest_v52.py — Varanus v5.2 Dual-Engine Full Backtest
Runs 5-fold WFV, collects all OOS trades, computes per-fold and aggregate metrics.
Outputs:
  varanus/config/varanus_v52_backtest_trades.csv  — full trade log
  varanus/config/varanus_v52_backtest_summary.csv — fold + aggregate metrics
"""
import json
import sys
...
PARAMS_FILE = _HERE / "varanus" / "config" / "best_params_v52.json"
OUT_TRADES  = _HERE / "varanus" / "config" / "varanus_v52_backtest_trades.csv"
OUT_SUMMARY = _HERE / "varanus" / "config" / "varanus_v52_backtest_summary.csv"
...
def run():
    print("=" * 65)
    print("  Varanus v5.2 Dual-Engine — Full Backtest")
    print("  Short Hunter (frozen v5.1) + Long Runner (Trial #87)")
    print("=" * 65)
    ...
    aggregate = {
        ...
        # fold-only columns — N/A for aggregate
        "cagr_pct":            None,
    ...
    # Align summary columns with fold results
    ...
    # ── Print report ─────────────────────────────────────────────────────────
    print("\n" + "=" * 65)
    print("  BACKTEST RESULTS — v5.2 Dual Engine")
    print("=" * 65)
