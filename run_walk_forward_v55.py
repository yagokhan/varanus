#!/usr/bin/env python3
"""
run_walk_forward_v52.py — Varanus v5.5 Dual-Engine Walk-Forward Validation
Validates optimized best_params_v55.json across 5-fold rolling OOS windows.
...
PARAMS_FILE = _HERE / "varanus" / "config" / "best_params_v55.json"
...
def run():
    print("=" * 60)
    print("  Varanus v5.5 Dual-Engine — Walk-Forward Validation")
    ...
    # Save fold summary
    out_csv = _HERE / "varanus" / "config" / "wfv_v55_results.csv"
    results_df.to_csv(out_csv, index=False)
    print(f"\n[+] Results saved to {out_csv.name}")

    # Save individual trades
    if not all_trades.empty:
        out_trades = _HERE / "varanus" / "config" / "wfv_v55_trades.csv"
