...
from varanus.model import VaranusModel, get_leverage, MODEL_CONFIG
...
CACHE_DIR  = Path("/home/yagokhan/chameleon/claude_code_project/data/cache")
STATE_FILE = _HERE / "config" / "paper_state.json"
TRADES_CSV = _HERE / "results"  / "paper_trades.csv"
ENV_FILE   = _HERE / "config"   / ".env"
...
class PaperTrader:
    ...
    def __init__(
        ...
        self.model: Optional[VaranusModel] = None
        ...
        # ccxt KuCoin — public endpoint, no keys needed for market data
        # (Binance.com is geo-blocked from this machine; KuCoin has identical data)
        self.exchange = ccxt.kucoin({"timeout": 15000, "enableRateLimit": True})
    ...
    def train(self) -> None:
        ...
        for asset in data_4h:
            ...
            try:
                ...
                X_all.append(X)
                y_all.append(y)
                logger.debug("  %-6s  rows=%-5d  win=%.1f%%",
                             asset, len(X), (y == 1).mean() * 100)
        ...
        self.model = VaranusModel(MODEL_CONFIG)
    ...
    def _get_live_data(self, asset: str) -> tuple[Optional[pd.DataFrame], Optional[pd.DataFrame]]:
        df_4h = self._fetch_live(asset, "4h", LIVE_BARS_4H)
        df_1d = self._fetch_live(asset, "1d", LIVE_BARS_1D)
        return df_4h, df_1d
    ...
    def scan(self) -> list[dict]:
        ...
        conf_thresh = self.params.get("confidence_thresh", 0.8147)
        ...
        for asset in TIER2_UNIVERSE:
            ...
            try:
                ...
                # TBM barriers using Optuna best params
                tbm_cfg = TBM_CONFIG.copy()
                tbm_cfg["take_profit_atr"]   = self.params.get("tp_atr_mult", 2.5)
                tbm_cfg["stop_loss_atr"]     = self.params.get("sl_atr_mult",  1.2)
    ...
    def check_exits(self) -> list[dict]:
        ...
        for asset, trade in list(open_trades.items()):
            df = self._fetch_live(asset, "4h", 10)
